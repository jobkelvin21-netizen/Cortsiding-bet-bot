#!/usr/bin/env python3
"""Manual slow game via Telegram → Groq arm → Confirm on Bet365 WS goal."""
...
def _normalize_bet365_id(raw: str) -> str:
    """Accept numeric id or OV… token; return digits the WS uses."""
    raw = (raw or "").strip()
    if not raw:
        return ""
...
class ArbitrageBot:
    ...
    async def start(self):
        # Telegram polling starts early so /login works before terminal finishes
