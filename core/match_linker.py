...
class MatchLinker:
    ...
    def __init__(
        ...
        # IMPORTANT:
        # This is intentionally the same dictionary object used by
        # SportyBetFeed.matches.
    ...
    # =========================================================================
    # FIND SPORTYBET MATCH
    # =========================================================================

    def _find_sportybet_match(
        self,
        home: str,
        away: str,
    ) -> Optional[dict]:
    ...
    def _notify_link(
        ...
        sb_home = sb_match.get(
            "home_team",
            "",
        )

        sb_away = sb_match.get(
            "away_team",
            "",
        )
    ...
    def _reconcile_once(self):
        ...
        poly_matches = (
            self.polymarket_feed.get_matches()
        )
        ...
        # -------------------------------------------------------------
        # REMOVE LINKS TO MATCHES THAT ARE NO LONGER LIVE
        # -------------------------------------------------------------
        ...
        for poly_id in list(self.links.keys()):
            ...
            if sb_id not in current_sb_ids:

                poly_match = next(
                    (
                        m
                        for m in poly_matches
                        if m.get("match_id") == poly_id
                    ),
                    None,
                )
        ...
        for poly_match in poly_matches:
            ...
            if (
                existing_link
                and existing_link.get("match_id")
                in current_sb_ids
            ):
                continue

            sb_match = self._find_sportybet_match(
                home,
                away,
            )
            ...
            if sb_match:
                ...
                logger.success(
                    f"[LINK] '{home}' vs '{away}' "
                    f"-> SportyBet match_id="
                    f"{sb_match.get('match_id')}"
                )
        ...
        if newly_linked or newly_unlinked:
            ...
            logger.info(
                f"[LINK] Reconcile: "
                f"{len(self.links)} active links "
                f"(+{newly_linked} new, "
                f"-{newly_unlinked} lost)"
            )
    ...
    async def _reconcile_loop(self):
        ...
        logger.info(
            "MatchLinker: starting continuous "
            "match-linking loop..."
        )
        ...
        while self.running:
            ...
            try:
                ...
                logger.error(
                    f"MatchLinker reconcile error: {e}"
                )
    ...
    async def on_polymarket(
        ...
        old_score = self.last_scores.get(
            poly_id
        )
        ...
        if new_score[0] > old_score[0]:
            ...
            logger.warning(
                f"[GOAL] Unable to determine scorer for "
                f"{home} {old_score[0]}-{old_score[1]} "
        ...
        sb_match = self.links.get(
            poly_id
        )

        # -------------------------------------------------------------
        # IMPORTANT:
        # If reconciliation has not linked it yet,
        # immediately try again here.
        # -------------------------------------------------------------

        if not sb_match:

            logger.info(
                f"[LINK] Goal detected before normal "
                f"reconciliation for '{home}' vs '{away}'. "
                f"Attempting immediate SportyBet lookup..."
            )
            ...
            if sb_match:
                ...
                logger.success(
                    f"[LINK] Immediate goal-time link: "
                    f"{home} vs {away} "
                    f"-> SportyBet "
                    f"{sb_match.get('match_id')}"
                )
        ...
        if not sb_match:
            ...
            logger.warning(
                f"⚠️ Goal detected for "
                f"'{home}' vs '{away}' "
                f"but no active SportyBet link exists."
            )
        ...
        if self.goal_callback:
            ...
            await self.goal_callback(
                {
                    "match_id": sb_match.get(
                        "match_id"
                    ),

                    "home_team": sb_match.get(
                        "home_team"
                    ),

                    "away_team": sb_match.get(
                        "away_team"
                    ),
