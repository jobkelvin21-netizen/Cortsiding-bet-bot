...
def try_extract_from_text(payload: str) -> Optional[dict]:
    """Try to extract basic match info from proprietary text frames"""
    try:
        # Common patterns that sometimes appear in Bet365 feeds
        # Example patterns (will improve as we see real useful frames)
        team_score = re.search(r'([A-Za-z0-9\s\.\-]{3,30})\s+(\d+)\s*[-:]\s*(\d+)\s+([A-Za-z0-9\s\.\-]{3,30})', payload)
        if team_score:
            return {
                "home_team": team_score.group(1).strip(),
                "home_score": int(team_score.group(2)),
                "away_score": int(team_score.group(3)),
                "away_team": team_score.group(4).strip(),
                "period": "",
            }
    except Exception:
        pass
...
class SchemaLearner:
    ...
    def _attempt_learn(self):
        ...
        home_field = away_field = score_field = period_field = None
...
class Bet365Feed:
    ...
    async def _launch_with_proxy(self):
        ...
        try:
            ...
            if any(sign in body_text.lower() for sign in blocked_signs) or any(sign in title.lower() for sign in blocked_signs):
                logger.warning(f"Proxy #{idx} got BLOCKED page")
    ...
    async def _run_once(self):
        ...
        try:
            ...
            # Try click Football
            try:
                football = await page.query_selector('div:has-text("Football"), span:has-text("Football")')
                if football:
